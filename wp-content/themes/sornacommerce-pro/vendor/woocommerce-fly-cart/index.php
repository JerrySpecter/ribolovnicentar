...
define( 'WOOSMC_VERSION', '1.5' );
define( 'WOOSMC_URI', get_template_directory_uri().'/vendor/woocommerce-fly-cart/' );
...
function woosmc_settings_page() {
	add_submenu_page( 'woocommerce', 'WooCommerce Fly Cart', 'Fly Cart', 'manage_options', 'woosmc', 'woosmc_settings_page_content' );
}
	...
	<div class="wrap" id="woosmc-settings-page">
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				<tr>
					<th scope="row">Show up after AJAX add to cart</th>
					<td>
						...
						<span class="description">The mini cart will be show up immediately after whenever click to AJAX Add to cart buttons (AJAX enable)? See <a
								...
								target="_blank">this setting</a></span>
					</td>
				</tr>
				<tr>
					<th scope="row">Show up after normal add to cart</th>
					<td>
				...
				<tr>
					...
					<td>
						...
						<p class="description">Fill class or id of the button, when click to this button the mini cart
				...
				<tr>
					<th scope="row">Show attributes of variation product</th>
					<td>
				...
				<tr>
					<th scope="row">Reload the cart on page load</th>
					<td>
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						...
						<span class="description">Hide the cart button if have no product in the cart?</span>
					</td>
				</tr>
				<tr>
					<th scope="row">Hide in Cart & Checkout page</th>
					<td>
...
function woosmc_get_cart_items( $style = '01' ) {
	...
	if ( sizeof( $items ) > 0 ) {
		...
		$cart_html .= '<div class="woosmc-area-top woosmc-items">';
		...
		if ( get_option( '_woosmc_buttons', '01' ) == '01' ) {
			$cart_html .= '<div class="woosmc-action"><div class="woosmc-action-inner"><div class="woosmc-action-left"><a href="' . WC()->cart->get_cart_url() . '">' . esc_html__( 'Cart', 'woosmc' ) . '</a></div><div class="woosmc-action-right"><a href="' . WC()->cart->get_checkout_url() . '">' . esc_html__( 'Checkout', 'woosmc' ) . '</a></div></div></div>';
		} else {
			if ( get_option( '_woosmc_buttons', '01' ) == '02' ) {
				$cart_html .= '<div class="woosmc-action"><div class="woosmc-action-inner"><div class="woosmc-action-full"><a href="' . WC()->cart->get_cart_url() . '">' . esc_html__( 'Cart', 'woosmc' ) . '</a></div></div></div>';
			}
			if ( get_option( '_woosmc_buttons', '01' ) == '03' ) {
				$cart_html .= '<div class="woosmc-action"><div class="woosmc-action-inner"><div class="woosmc-action-full"><a href="' . WC()->cart->get_checkout_url() . '">' . esc_html__( 'Checkout', 'woosmc' ) . '</a></div></div></div>';
			}
		}
		...
		$cart_html .= '<div class="woosmc-no-item">' . esc_html__( 'Have no product in the cart!', 'woosmc' ) . '</div>';
	}
...
function woosmc_wp_footer() {
	...
	<?php if ( get_option( '_woosmc_count', 'yes' ) == 'yes' ) {
		$woosmc_count      = WC()->cart->get_cart_contents_count();
		$woosmc_count_hide = '';
		if ( ( get_option( '_woosmc_count_hide_empty', 'no' ) == 'yes' ) && ( $woosmc_count <= 0 ) ) {
			$woosmc_count_hide = 'woosmc-count-hide';
		}
		if ( ( get_option( '_woosmc_count_hide_checkout', 'no' ) == 'yes' ) && ( is_cart() || is_checkout() ) ) {
			$woosmc_count_hide = 'woosmc-count-hide';
		}
		?>
		<div id="woosmc-count"
		     class="woosmc-count <?php echo esc_attr( 'woosmc-count-' . get_option( '_woosmc_count_position', 'bottom-left' ) ); ?><?php echo( ( $woosmc_count_hide != '' ) ? ' ' . esc_attr( $woosmc_count_hide ) : '' ); ?>">
			<i class="<?php echo esc_attr( get_option( '_woosmc_count_icon', 'woosmc-icon-cart7' ) ); ?>"></i>
			<span id="woosmc-count-number"><?php echo esc_attr( $woosmc_count ); ?></span>
		</div>
	<?php } ?>
