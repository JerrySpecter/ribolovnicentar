  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<input type="text" name="'. $this->element_name( '[font]' ) .'" class="cs-typo-font hidden" data-atts="font" value="'. $value['font'] .'" />';
        ...
        echo '<label class="cs-typography-variant">';
        echo '<select name="'. $this->element_name( '[size]' ) .'" data-atts="size"><option>Font size</option>';
        for ($x = 10; $x <= 80; $x++) {
		...
		$line_height = (isset($line_height) && !empty($line_height))? $line_height : $defaults_value['line_height'];
		
        echo '<label class="cs-typography-variant">';
        echo '<select name="'. $this->element_name( '[line_height]' ) .'" data-atts="size">
			<option>line height</option>
		';
        for ($x = 10; $x <= 120; $x++) {
          echo '<option value="'. $x .'"'. $this->checked( $x, $line_height, 'selected' ) .'>'. $x  .' px </option>';
        }
        echo '</select>';
        echo '</label>';
