...
if ( ! function_exists( 'sornacommerce_customize_search_form' ) ) :
	 ...
	 */
	function sornacommerce_customize_search_form() {

		$form = '<form role="search" method="get" class="search-form widget-search" action="' . esc_url( home_url( '/' ) ) . '">
			
			<input type="search" class="search-field" placeholder="' . esc_attr_x( 'Search&hellip;', 'placeholder', 'sornacommerce' ) . '" value="' . get_search_query() . '" name="s" title="' . esc_attr_x( 'Search for:', 'label', 'sornacommerce' ) . '" />
			</label>
...
add_filter( 'get_search_form', 'sornacommerce_customize_search_form', 15 );
