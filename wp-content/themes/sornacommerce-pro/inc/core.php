...
function sornacommerce_setup() {
	...
	if(cs_get_option('woo_single_gallery_zoom')== true){
	add_theme_support( 'wc-product-gallery-zoom' );
	} if(cs_get_option('woo_single_gallery_lightbox')== true){
	add_theme_support( 'wc-product-gallery-lightbox' );
	} if(cs_get_option('woo_single_gallery_slider')== true){
...
function sornacommerce_key_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'sornacommerce_key_content_width', 900 );
}
...
function sornacommerce_scripts() {
	...
	wp_enqueue_style( 'bootstrap', get_theme_file_uri( '/assets/css/bootstrap.min.css' ), array(), '3.3.7' );
	wp_enqueue_style( 'OwlCarousel-css', get_theme_file_uri( '/vendor/owl-carousel/owl.carousel.css' ), '1.3.3' );
	wp_enqueue_style( 'OwlCarousel-css-theme', get_theme_file_uri( '/assets/css/magnific-popup.css' ), '1.3.3' );
	wp_enqueue_style( 'OwlCarousel-css-transitions', get_theme_file_uri( '/vendor/owl-carousel/owl.transitions.css' ), '1.3.3' );
	wp_enqueue_style( 'font-awesome', get_theme_file_uri( '/assets/css/font-awesome.min.css' ), '4.7.0' );
	
	wp_enqueue_style( 'woocommerce-layout', get_theme_file_uri( '/assets/css/woocommerce-layout.css' ), array(), '3.3.7' );
	wp_enqueue_style( 'sornacommerce-style', get_stylesheet_uri() );
	wp_enqueue_style( 'sornacommerce-pro-dynamic', get_theme_file_uri( '/styling/style.php' ), '1.0' );

		
	wp_enqueue_script( 'bootstrap-js', get_theme_file_uri( '/assets/js/bootstrap.min.js' ), array('jquery'), '3.3.4', true );
	
	wp_enqueue_script( 'OwlCarousel', get_theme_file_uri( '/vendor/owl-carousel/owl.carousel.min.js' ), 0,0,true );
	wp_enqueue_script( 'sornacommerce-masonry', get_theme_file_uri( '/vendor/masonry.pkgd.min.js' ), 0,0,true );
	
	wp_enqueue_script( 'jquery.sticky', get_theme_file_uri( '/vendor/jquery.magnific-popup.js' ),0,0,true );
	
	wp_enqueue_script( 'sornacommerce-js', get_theme_file_uri( '/assets/js/init.js' ), 0,0,true );
	wp_enqueue_script( 'sornacommerce-dynamic-js', get_theme_file_uri( '/styling/dynamc_js.php' ),0,0,true );
...
function sornacommerce_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'WooCommerce Sidebar', 'sornacommerce' ),
		'id'            => 'woocommerce',
		'description'   => esc_html__( 'Add widgets here.', 'sornacommerce' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
